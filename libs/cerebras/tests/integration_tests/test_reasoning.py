...
def test_gpt_oss_reasoning_content() -> None:
    ...
    response = llm.invoke(
        ...
        "Station B (120 miles east of A) at 40 mph heading west, at what time will they meet "
        "if the first train left at 2:00 PM?"
    )
    ...
    # Note: With high reasoning effort, models may return only reasoning without separate text content
    # The reasoning itself contains the answer, which is valid behavior
...
def test_gpt_oss_reasoning_streaming() -> None:
    ...
    for chunk in llm.stream(
        "What is the cube root of 50.653? Show your reasoning."
    ):
    ...
    print(f"Streamed Reasoning: {full_reasoning}")
...
def test_zai_glm_reasoning_content() -> None:
    ...
    response = llm.invoke(
        "Explain the key differences between quantum computing and classical computing, "
        "focusing on how information is processed."
    )
...
def test_zai_glm_reasoning_streaming() -> None:
    ...
    # This is valid behavior for reasoning models
    print(f"Streamed Reasoning: {full_reasoning[:200]}...")
    if full_text:
        print(f"Streamed Text: {full_text}")
